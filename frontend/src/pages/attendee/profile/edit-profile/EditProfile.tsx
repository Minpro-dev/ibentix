...
// //       try {
// //         const response = await axiosInstance.patch('/update-details');
...
// //   return (
// //     <DashboardLayout user={user} isEditMode={isEditMode} setIsEditMode={setIsEditMode}>
// //       <ProfilePage isEditMode={isEditMode} />
// //     </DashboardLayout>
// //   );
// // }
...
// import { ProfileSchema } from "../../../organizer/profile/schemas/profileSchema";
...
function validateFields(fields: EditUserPayload & { confirmPassword?: string }) {
  const errors: Record<string, string> = {};

  if (fields.firstName !== undefined) {
    if (fields.firstName.length < 2) errors.firstName = 'First name must be at least 2 characters';
    if (fields.firstName.length > 30) errors.firstName = 'First name must be at most 30 characters';
  }
  if (fields.lastName !== undefined) {
    if (fields.lastName.length < 2) errors.lastName = 'Last name must be at least 2 characters';
    if (fields.lastName.length > 30) errors.lastName = 'Last name must be at most 30 characters';
  }
  if (fields.phone !== undefined && fields.phone !== '') {
    if (fields.phone.length < 9) errors.phone = 'Phone must be at least 9 digits';
    if (fields.phone.length > 15) errors.phone = 'Phone must be at most 15 digits';
  }
  if (fields.address !== undefined && fields.address !== '') {
    if (fields.address.length < 5) errors.address = 'Address must be at least 5 characters';
    if (fields.address.length > 45) errors.address = 'Address must be at most 45 characters';
  }
  if (fields.password) {
    ...
    <div className="flex flex-col items-center gap-4">
      <div className="relative group">
        {src ? (
          ...
          <div className="w-28 h-28 rounded-full bg-blue-100 flex items-center justify-center text-blue-700 text-3xl font-bold border-4 border-white shadow-lg">
            {initials}
          </div>
        )}
      ...
      <p className="text-xs text-gray-400 text-center">JPG, PNG or WebP · Max 800KB</p>
    </div>
  );
}
...
function Toast({ message, type }: { message: string; type: 'success' | 'error' }) {
  return (
    <div className={`fixed top-6 right-6 z-50 px-5 py-3 rounded-xl shadow-lg text-sm font-semibold flex items-center gap-2 animate-in slide-in-from-top-2
...
export default function EditProfilePage() {
  ...
  const mutation = useMutation({
    mutationFn: async () => {
      ...
      if (firstName !== profile?.firstName) formData.append('firstName', firstName);
      ...
      if (address !== (profile?.address || '')) formData.append('address', address);
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6 pb-24 px-4">
      ...
      {/* Avatar */}
      <Section title="Profile Photo" subtitle="Click the photo to upload a new one">
        <AvatarUpload
      ...
      {/* Personal Info */}
      <Section title="Personal Information" subtitle="All fields are optional — only changed fields will be saved">
        ...
        <div>
          ...
          <select
            ...
            className="w-full px-4 py-3 rounded-xl border border-gray-200 bg-white text-sm text-gray-900 focus:border-blue-500 focus:ring-2 focus:ring-blue-100 outline-none transition-all"
          >
      ...
      <div className="flex flex-col sm:flex-row justify-end gap-3 pb-4">
        ...
        <button
          ...
          disabled={mutation.isPending}
          className="px-8 py-3 rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-semibold text-sm transition-all shadow-sm disabled:opacity-60 flex items-center gap-2"
        >
          {mutation.isPending && (
