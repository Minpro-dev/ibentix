    ...
    >
      <div className="relative aspect-[4/3] overflow-hidden">
        ...
        {isEnded ? (
          ...
          <div className="absolute top-4 left-4">
            <span className={cn(
              ...
              event.badge.includes('Limited') ? "bg-indigo-100 text-brand-primary" : "bg-orange-100 text-orange-700"
            )}>
              {event.badge}
            </span>
          </div>
        ...
        <div className={cn(
          ...
          <Heart className={cn("w-5 h-5", isEnded ? "text-gray-400" : "fill-red-500 text-red-500")} />
        </div>
      </div>
      ...
      <div className="p-6">
        <h3 className={cn(
          ...
          isEnded ? "text-gray-500" : "text-gray-900 group-hover:text-brand-primary"
        )}>
          {event.title}
        </h3>
        ...
        <div className="flex items-center justify-between pt-4 border-t border-gray-100">
          <div>
            <p className="text-[10px] text-gray-400 uppercase font-bold tracking-wider mb-0.5">
              {event.id === '1' ? 'Starting from' : event.id === '2' ? 'Admission' : 'Ticket'}
            </p>
            <p className={cn(
